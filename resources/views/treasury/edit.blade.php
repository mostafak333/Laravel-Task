    ...
    <div class="col-sm">
        <div class="card mb-4 box-shadow ">
            <div class="card-header bg-primary text-white">
                <h4 class="my-0 font-weight-normal">Edit Treasury</h4>
            </div>
            <div class="card-body">
                <form action="{{route('treasury.update',$treasury->id,)}}" method="post">
                    ...
                    <div class="row">
                        <div class="col-md-3 mb-3">
                            ...
                            <select id="status" class="form-select" aria-label="Default select example" name="status"
                                required>
                                <option selected value="{{$treasury->status}}">{{ ($treasury->status == 0) ? 'Internal'
                                    : 'External'}}</option>
                    ...
                    $treloc = DB::select("SELECT * From `location` WHERE id =$treasury->locationId ")
                    @endphp
                    <div class="row">
                        <div class="col-md-3 mb-3">
                            ...
                            <select id="location" class="form-select" aria-label="Default select example"
                                ...
                                <option selected value="{{$treasury->locationId}}">{{$treloc[0]->id}}
                                    {{$treloc[0]->country}}->{{$treloc[0]->governorate}}->{{$treloc[0]->address}}
                                </option>
                    ...
                    <div class="row">
                        <div class="col-md-3 mb-3">
                            ...
                            <input type="text" class="form-control" name="weight" placeholder=""
                                value="{{$treasury->weight}}" required>
                        </div>
                    </div>
